"""ComfyUI client (REST + history polling)."""
...
class ComfyClient:
    ...
    def upload_image(self, path: str | Path) -> str:
        """Upload a local image into ComfyUI's input dir; returns the filename."""
    ...
    def wait(self, prompt_id: str, timeout_s: float = 1800.0, poll_interval: float = 3.0) -> dict:
        ...
        Queue-aware: while the prompt sits in the queue the wait is allowed to
        exceed ``timeout_s``; once it leaves the queue without completing, the
        wait gives up after ``timeout_s`` of no progress.
        """
