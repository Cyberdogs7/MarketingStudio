"""Krea 2 workflow loading + parameterization.

Loads the exported API workflow (``workflows/image_keyframe.json``), sets the
dynamic values, and runs a t2i generation through a ComfyClient. The LLM
'Refine Prompt' node and the LoRA switch are turned OFF so the prompt we compile
is exactly what the model conditions on - deterministic, no LLM in the loop.
"""
...
def build_keyframe_ref_workflow(workflow: dict[str, Any], prompt: str, seed: int,
    ...
    so the generated keyframe inherits the referenced identities (creator ref,
    product ref). ``weights`` (one per ref) overrides the flat ``weight``.
    """
    wf = copy.deepcopy(workflow)
    wf[NODE_PROMPT]["inputs"]["value"] = prompt
    wf[NODE_SEED]["inputs"]["seed"] = resolve_seed(seed)
