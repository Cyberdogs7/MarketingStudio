"""MiniMax H3 shot generation via ComfyUI (ref2va + Retake Stitch).

Builds the H3 workflows programmatically from the node schemas, runs a ref2va
shot (references as sockets -> <Picture N> / <Audio N>) or a retake-extension
(MiniMaxH3RetakeStitchCS) that appends new content onto a base video, and saves
the MP4. Ported from the anime studio; used here for photoreal UGC output.
"""
...
def build_h3_shot_workflow(
    ...
    """MiniMaxH3DirectorCS workflow (t2v or ref2va)."""
    ...
    if ref_lora:
        ...
        wf["2a"] = {"class_type": "LoraLoaderModelOnly",
                    "inputs": {"lora_name": ref_lora, "strength_model": 0.75,
                               "model": ["2", 0]}}
...
def build_h3_ref2va_workflow(
    ...
    """Official Comfy-Org ref2va workflow (MiniMaxH3ReferenceToVideo).

    Refs are wired as SOCKETS (LoadImage -> ref_image_0..8, LoadAudio ->
    ref_audio_0..2) and the node auto-labels them <Picture N> / <Audio N> in
    connection order, which the prompt references directly.
    """
    cfg = cfg or get_config()
    ck = cfg.get("comfy", "checkpoints", {})
    k, frames, _ = snap_duration(duration_s)
...
def build_h3_retake_workflow(
    ...
    """Build a Retake workflow: append/extend a marked range of a video.
    ...
    extension (head + new content, no tail) - this is the continuous-take
    stitch: each shot appends onto the previous stitched video.

    `audio_segments` (optional) are timeline entries like
    `{"audioFile": <name>, "trimStart": 0, "start": 0, "length": <frames>}` that
    wire a voice-timbre reference into the retake (`use_custom_audio` is enabled
    automatically), so the extension keeps the same voice.
    """
    cfg = cfg or get_config()
    ck = cfg.get("comfy", "checkpoints", {})
